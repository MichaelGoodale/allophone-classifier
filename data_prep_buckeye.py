...
err = 0
for speaker in os.listdir(s.Buckeye_DIR):
    conversations = set(x.split('.')[0] for x in os.listdir(os.path.join(s.Buckeye_DIR, dialect_region, speaker)))
    for conversation in conversations:
        new_stop_dict, new_err = extract_wav_files(os.path.join(s.Buckeye_DIR, dialect_region, speaker, conversation))
        err += new_err
